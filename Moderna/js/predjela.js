function dohvatiSvaPredjela(){
    ...
    for( let i=0; i < recepti.length; i++){
        if(recepti[i]["grupa"] == "predjelo"){
            predjela.push(recepti[i]);
        }
    }
    if( predjela.length == 0){
        return null;
    }
    else return predjela;
}
...
$(document).ready(function(){
    ...
    let svaPredjela = dohvatiSvaPredjela();

    $("#red-predjela").empty();

    if( svaPredjela != null ){
        for( let i=0; i < svaPredjela.length; i++){
            ...
            let slika = $("<img>").attr("src","../slike/" +  svaPredjela[i]["slike"][0]).addClass("img-fluid");
            divSlika.append(slika);

            artikl.append(divSlika);

            let h2 = $("<h2></h2>").addClass("entry-title");
            let a1 = $("<a></a>").attr("href","#").attr("name", svaPredjela[i]["id"]).addClass("receptLink");
            a1.append(svaPredjela[i]["naziv"]);
            ...
            let prosecnaOcena = izracunajProsecnuOcenu(svaPredjela[i]["id"]);
            ...
            a3.append(svaPredjela[i]["trajanje"] + "min");
            li3.append(i3);
            li3.append(a3);
            ...
            a9.append(svaPredjela[i]["tezina"]);
            li4.append(i4);
            li4.append(a9);
