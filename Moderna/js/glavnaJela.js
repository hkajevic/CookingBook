function dohvatiSvaGlavnaJela(){
    ...
    for( let i=0; i < recepti.length; i++){
        if(recepti[i]["grupa"] == "glavno jelo"){
            glavnaJela.push(recepti[i]);
        }
    }
    if( glavnaJela.length == 0){
        return null;
    }
    else return glavnaJela;
}
...
let svaGlavnaJela = dohvatiSvaGlavnaJela();

$("#red-glavna-jela").empty();

if( svaGlavnaJela != null ){
    for( let i=0; i < svaGlavnaJela.length; i++){
        ...
        let slika = $("<img>").attr("src","../slike/" +  svaGlavnaJela[i]["slike"][0]).addClass("img-fluid");
        divSlika.append(slika);

        artikl.append(divSlika);

        let h2 = $("<h2></h2>").addClass("entry-title");
        let a1 = $("<a></a>").attr("href","#").attr("name", svaGlavnaJela[i]["id"]).addClass("receptLink");
        a1.append(svaGlavnaJela[i]["naziv"]);
        ...
        let prosecnaOcena = izracunajProsecnuOcenu(svaGlavnaJela[i]["id"]);
        ...
        a8.append(svaGlavnaJela[i]["dodao"]);
        li1.append(i1);
        li1.append(a8);
        ...
        a3.append(svaGlavnaJela[i]["trajanje"] + "min");
        li3.append(i3);
        li3.append(a3);
        ...
        a9.append(svaGlavnaJela[i]["tezina"]);
        li4.append(i4);
        li4.append(a9);
        ...
        $("#red-glavna-jela").append(glavniDiv);
    }
