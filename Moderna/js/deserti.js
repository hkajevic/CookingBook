...
$(document).ready(function(){
    ...
    let sviDeserti = dohvatiSveDeserte();

    $("#red-deserti").empty();

    if( sviDeserti != null ){
        for( let i=0; i < sviDeserti.length; i++){
            ...
            let slika = $("<img>").attr("src","../slike/" +  sviDeserti[i]["slike"][0]).addClass("img-fluid");
            divSlika.append(slika);

            artikl.append(divSlika);

            let h2 = $("<h2></h2>").addClass("entry-title");
            let a1 = $("<a></a>").attr("href", "#").attr("name", sviDeserti[i]["id"]).addClass("receptLink");
            a1.append(sviDeserti[i]["naziv"]);
            ...
            let prosecnaOcena = izracunajProsecnuOcenu(sviDeserti[i]["id"]);
            ...
            a3.append(sviDeserti[i]["trajanje"] + " min");
            li3.append(i3);
            li3.append(a3);
